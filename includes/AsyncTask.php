 ...
 * Originally written by {@link https://github.com/techcrunch TechCrunch} and
...
abstract class AsyncTask
{
    ...
    public function __construct($auth_level = self::BOTH)
    {
        ...
        add_action(
            ...
            [$this, 'launch'],
            (int) $this->priority,
            (int) $this->argumentCount
        );

        if ($auth_level & self::LOGGED_IN) {
            add_action(
                "admin_post_wp_async_$this->action",
                [$this, 'handlePostback']
            );
        }
        ...
        if ($auth_level & self::LOGGED_OUT) {
            add_action(
                "admin_post_nopriv_wp_async_$this->action",
                [$this, 'handlePostback']
            );
        }
    }
     ...
     * get an exception thrown by prepareData().
     *
    ...
    public function launch()
    {
        ...
        $data['_nonce'] = $this->createAsyncNonce();

        $this->bodyData = $data;

        if (!has_action('shutdown', [$this, 'launchOnShutdown'])) {
            add_action('shutdown', [$this, 'launchOnShutdown']);
        }
    }
    ...
    {
        if (isset($_POST['_nonce']) && $this->verifyAsyncNonce($_POST['_nonce'])) {
